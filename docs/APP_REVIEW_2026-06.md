...
### [x] 23. Onboarding completion loop — ROOT CAUSE: missing profile row
**Status:** CODE DONE — ⚠️ USER MUST RUN `supabase/fix_missing_profiles.sql`. Diagnostic proved it: `/home` `get_session_data` returned `me: null` (no error) for auth user af3968a5 (bukkyn@gmail.com) → no `profiles` row existed. The `handle_new_user` signup trigger swallows errors (`exception when others then return new`), so the profile silently never got created. With no row, `create_couple_for_user`'s `update profiles set couple_id` matched 0 rows (but still returned an invite code → finish screen looked fine), so `couple_id` was never stored → loop. Orphan couples piled up (4). FIX: backfill profiles for all auth.users missing one + made `create_couple_for_user`/`join_couple_for_user` self-healing (`insert ... on conflict do nothing` before linking). schema.sql updated; layout diagnostic reverted to normal redirect. The earlier getUser()+throw-on-error layout change stays (good hardening). Item 22 below = the UI polish (scroll/keyboard/dark/bloom).
