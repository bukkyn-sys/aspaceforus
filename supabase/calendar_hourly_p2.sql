-- ════════════════════════════════════════════════════════════════════════════
-- Calendar overhaul — Phase 2: day-part availability.
--
-- Availability moves from one whole-day free row to up to four PART rows per day
-- (morning / afternoon / evening / night). Presence of a row = free that part.
-- Overlap "free together" is per-part. A day can be partially together.
--
-- Includes: schema change + lossless backfill (every existing free day → its four
-- parts), new set_availability / set_availability_day RPCs, and get_home_data's
-- free-days → free-WINDOWS change.
--
-- Idempotent / re-runnable. Run AFTER calendar_hourly_p1.sql.
-- ════════════════════════════════════════════════════════════════════════════

-- 1. Add the part column; relax status so part-rows don't need it.
alter table availability add column if not exists part text;
alter table availability alter column status drop not null;

-- 2. Drop the old 3-column unique (couple,user,date) — robustly, by shape, since
--    its generated name may vary. Required before multiple parts can share a date.
do $$
declare c text;
begin
...
-- 3. Lossless backfill: expand each legacy whole-day free row into four part-rows.
do $$
begin
  if exists (select 1 from availability where part is null) then
    insert into availability (couple_id, user_id, date, part, status)
    select a.couple_id, a.user_id, a.date, p.part, 'free'
    from availability a
    ...
    delete from availability where part is null;   -- drop legacy non-free/busy rows
  end if;
end $$;
...
-- 5. RPCs. Replace the whole-day set_availability with a per-part toggle, plus a
--    convenience all-day toggle.
...
begin
  ...
  if p_free then
    ...
    delete from availability
      where couple_id = p_couple_id and user_id = p_user_id and date = p_date and part = p_part;
  end if;
end; $$;
...
-- ════════════════════════════════════════════════════════════════════════════
-- 6. get_home_data — free DAYS → free WINDOWS. Identical to the daily-embed
--    version except v_free now returns the next 3 (date, part) overlaps.
-- ════════════════════════════════════════════════════════════════════════════
...
begin
  ...
  -- Next 3 free WINDOWS (date + part) where BOTH partners are free, next 60 days.
