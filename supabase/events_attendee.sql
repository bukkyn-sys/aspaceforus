-- ════════════════════════════════════════════════════════════════════════════
-- Events: who's attending. attendee = a single profile id (just that person),
-- or NULL = both of you. Reuses the owner-identity model (resolveOwner).
-- get_home_data now carries attendee / parts / start_time on events so the home
-- "events" module can open the same edit form as the calendar.
-- Idempotent. Run AFTER calendar_dayparts_rebuild.sql.
-- ════════════════════════════════════════════════════════════════════════════

alter table events add column if not exists attendee uuid references profiles(id);
