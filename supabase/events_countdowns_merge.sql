-- ════════════════════════════════════════════════════════════════════════════
-- Amalgamate countdowns into events. ONE concept now: events. Every event counts
-- down on home; nothing is called a "countdown" any more. The separate countdowns
-- table is retired — its rows migrate into `events` as all-day events, and
-- get_home_data feeds the home "events" module from `events`.
--
-- Idempotent: once the countdowns table is gone, the migration block is a no-op.
-- Supabase runs the whole script as one transaction, so a failure rolls back.
-- Run AFTER dashboard_layout.sql.
-- ════════════════════════════════════════════════════════════════════════════

-- ── 1. get_home_data: the home "events" module now reads from `events` ─────────
-- (Returned under the existing `countdowns` key, shaped with target_date/end_date,
--  so the client's days-until badge keeps working untouched.)
...
-- ── 2. Migrate countdowns → events, then drop the retired table ───────────────
do $$
begin
