-- ════════════════════════════════════════════════════════════════════════════
-- Fix: mood timestamp + complete get_home_data shape.
--
-- Two bugs, one migration:
--   1. update_my_mood never wrote profiles.mood_updated_at, so the "Xm ago"
--      label under each partner's mood only survived as long as the live
--      broadcast's `at` was in memory — any reload (window re-entry, partner
--      activity, pull-to-refresh) read the null column and the label vanished.
--   2. The Home screen reads note_items / dashboard_layout / events-with-parts
--      from get_home_data, but an older redefinition (vault_priority_todo.sql)
--      dropped note_items and read events from the legacy countdowns table.
--      When that version is the one deployed, every reload returns no note
--      lines and the shared note appears to clear itself "after a while".
--
-- This re-applies the complete, current get_home_data (matching
-- events_attendee.sql) and makes update_my_mood stamp the time.
--
-- Idempotent. Safe to run any time; run AFTER events_attendee.sql / vault_*.
-- ════════════════════════════════════════════════════════════════════════════

-- ── 1. Mood: stamp the update time ──────────────────────────────────────────
...
begin
  ...
  update profiles set current_mood = p_mood, mood_updated_at = now() where id = p_user_id;
end; $$;

-- ── 2. get_home_data — complete shape (note_items + dashboard_layout + parts) ─
...
begin
  ...
  -- Home "events" module (kept under the `countdowns` key). Carries every field
  -- the edit form needs.
