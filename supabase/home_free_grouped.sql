-- ════════════════════════════════════════════════════════════════════════════
-- Home dashboard — condense "next free times" to one line per day.
-- get_home_data.free_days now returns the next 3 DAYS that have any both-free
-- part, each as { date, parts: [parts] } (e.g. Wed → ['morning','afternoon']),
-- instead of one row per (date, part). Everything else is unchanged.
-- Idempotent. Run after run_vault_calendar.sql.
-- ════════════════════════════════════════════════════════════════════════════
...
begin
  ...
  -- Next 3 days with any both-free part; each carries its ordered parts list.
  select coalesce(json_agg(json_build_object('date', g.d, 'parts', g.parts) order by g.d), '[]'::json) into v_free
  from (
    select dp.d,
           array_agg(dp.prt order by dp.ord) as parts
    from (
