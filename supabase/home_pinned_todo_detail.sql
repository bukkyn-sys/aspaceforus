-- ════════════════════════════════════════════════════════════════════════════
-- Richer pinned-to-do on Home.
--
-- The Home "pinned list" card needs to:
--   • keep an item visible (ticked + struck through) after you check it, even
--     across a navigation/reload — not vanish. So we now also return items that
--     were completed TODAY, not just open ones.
--   • show who each item is for (assignee) and, for "both must tick" items, the
--     0/2 · 1/2 progress. So we return needs_both + ticked_by + done.
--
-- Implemented as a small helper (priority_todo_json) so get_home_data just calls
-- it — no need to re-paste the whole RPC. get_home_data is then re-created to use
-- the helper (its body is otherwise identical to fix_mood_timestamp_and_home_data).
--
-- Idempotent. Run AFTER fix_mood_timestamp_and_home_data.sql.
-- ════════════════════════════════════════════════════════════════════════════

create or replace function priority_todo_json(p_list_id uuid)
returns json language sql stable security definer set search_path = public as $$
  select json_build_object(
    'list_id', tl.id, 'title', tl.title, 'emoji', tl.emoji,
    'remaining', (select count(*) from vault_todos where list_id = tl.id and done = false and parent_id is null),
    'items', coalesce((
      select json_agg(json_build_object(
        'id', t.id, 'title', t.title, 'due_date', t.due_date, 'assignee', t.assignee,
        'needs_both', t.needs_both, 'ticked_by', t.ticked_by, 'done', t.done
      ) order by t.done, t.created_at)
      from (
        -- Open items, plus anything finished today so a fresh tick still shows.
        select id, title, due_date, assignee, needs_both, ticked_by, done, created_at
        from vault_todos
        where list_id = tl.id and parent_id is null
          and (done = false or done_at >= current_date)
        order by done, created_at
        limit 8
      ) t
    ), '[]'::json)
  )
  from vault_todo_lists tl where tl.id = p_list_id;
$$;

-- ── get_home_data — same as fix_mood_timestamp_and_home_data, priority via helper
